...
class HangmanGame:
    def __init__(self, master):
            ...
            self.hints_label = tk.Label(master, text=self.get_hint())
            self.hints_label.pack() # It gives a hint at window

            self.incorrect_label = tk.Label(master, text="Incorrect letters: ")
            self.incorrect_label.pack() # It shows the incorrect letters at window

            self.lives_label = tk.Label(master, text=f"Lives: {self.tries}")
            self.lives_label.pack() # Num of lives at window

            self.entry_label = tk.Label(master, text="Enter a letter:")
            ...
            self.submit_button = tk.Button(master, text="Submit", command=self.submit_letter)
...
# Shows hints for each word
    def get_hint(self):
        hints = {
            ...
            "dino": "Don't move, it sees the movement",
    ...
    def update_labels(self):
        ...
        self.incorrect_label.config(text="Incorrect letters: " + " ".join(self.wrong_letters))
        self.lives_label.config(text=f"Lives: {self.tries}")
